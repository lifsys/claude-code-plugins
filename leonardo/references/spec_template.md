...
Complete XML template for generating comprehensive project specifications. This template captures ALL elements needed for a production-ready application specification.
...
**CRITICAL**: All output MUST use XML tags with `-` list items exactly as shown in this template.
...
<project_specification>
  ...
  <overview>
    ...
    Mention the UI framework and design approach.]
  </overview>

  <technology_stack>
    <api_key>
        [Instructions for API key location/management. Example:
        "You can use an API key located at /tmp/api-key for testing.
        You will not be allowed to read this file, but you can reference it in code."]
    </api_key>
    <frontend>
      ...
      <state_management>[React hooks and context | Redux | Zustand | Pinia | etc.]</state_management>
      <routing>[React Router for navigation | Vue Router | SvelteKit routing | etc.]</routing>
      <markdown>[React Markdown for message rendering | marked | etc.]</markdown>
      <code_highlighting>[Syntax highlighting for code blocks | Prism | highlight.js | Shiki]</code_highlighting>
    ...
    <backend>
      ...
      <api_integration>[Claude API for chat completions | OpenAI API | etc.]</api_integration>
      <streaming>[Server-Sent Events for streaming responses | WebSockets | none]</streaming>
    </backend>
    <communication>
      <api>[RESTful endpoints | GraphQL | tRPC]</api>
      <streaming>[SSE for real-time message streaming | WebSockets]</streaming>
      <claude_api>[Integration with Claude API using Anthropic SDK | etc.]</claude_api>
    </communication>
  </technology_stack>

  <prerequisites>
    <environment_setup>
      - [Repository includes .env with API_KEY configured]
      - [Frontend dependencies pre-installed via pnpm/npm]
...
These tags describe the app being built. Rename when the base name doesn't apply:

| Base Pattern | Adapt To | When Project Is... |
|--------------|----------|-------------------|
| `<chat_interface>` | `<command_interface>`, `<product_browsing>`, `<dashboard_view>`, `<content_editor>` | CLI, e-commerce, analytics, CMS |
| `<artifacts>` | `<output_generation>`, `<cart_checkout>`, `<report_panel>`, `<preview_panel>` | CLI, e-commerce, analytics, CMS |
| `<conversation_management>` | `<session_management>`, `<order_management>`, `<report_management>`, `<content_management>` | CLI, e-commerce, analytics, CMS |
| `<projects>` | `<profiles>`, `<wishlists>`, `<dashboards>`, `<sites>` | CLI, e-commerce, analytics, CMS |
| `<custom_instructions>` | `<config_files>`, `<preferences>`, `<data_sources>`, `<templates>` | CLI, e-commerce, analytics, CMS |

---

## Adding Project-Specific Tags

When a project requires additional functionality not covered by the base tags, add new tags following the same format:
...
Place project-specific tags in the appropriate section (e.g., within `<core_features>` for new feature categories).
